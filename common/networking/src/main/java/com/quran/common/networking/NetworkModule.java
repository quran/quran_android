...
public class NetworkModule {
  ...
  static OkHttpClient provideOkHttpClient(Dns dns) {
    HandshakeCertificates certificates = provideHandshakeCertificates();
  ...
  @Provides
  @Singleton
  static HandshakeCertificates provideHandshakeCertificates() {
    try {
      String certificate = readCertificate();
      CertificateFactory certificateFactory = CertificateFactory.getInstance("X.509");
      X509Certificate isgCertificate = (X509Certificate) certificateFactory.generateCertificate(
          new ByteArrayInputStream(certificate.getBytes("UTF-8")));

      return new HandshakeCertificates.Builder()
          .addTrustedCertificate(isgCertificate)
          .addPlatformTrustedCertificates()
          .build();
    } catch (Exception e) {
      throw new RuntimeException("Failed to create HandshakeCertificates", e);
    }
  }
