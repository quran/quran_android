...
class AudioRecorder {
    ...
    try {
      ...
      while (isRecording) {
        ...
        if (readCount > 0) {
          ...
          synchronized(samplesLock) {
            appendToSampleBuffer(floats, readCount)
          }
  ...
  private fun appendToSampleBuffer(source: FloatArray, count: Int) {
    val required = sampleCount + count
    if (required > MAX_BUFFER_SIZE) return
    if (required > sampleBuffer.size) {
      val newSize = minOf(maxOf(sampleBuffer.size * 2, required), MAX_BUFFER_SIZE)
      sampleBuffer = sampleBuffer.copyOf(newSize)
    }
    source.copyInto(sampleBuffer, sampleCount, 0, count)
    sampleCount += count
  }
  ...
  companion object {
    ...
    private const val MAX_BUFFER_SIZE = SAMPLE_RATE * 120 // 2 minutes max
  }
}
