...
class QuranAudioService : MediaBrowserServiceCompat(), MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
  @Inject
  lateinit var quranInfo: QuranInfo
  private lateinit var mSession: MediaSessionCompat
  private var job: Job? = null
  var mediaPlayer: MediaPlayer? = null
  var loadedMediaItem: MediaItem? = null
  ...
  private inner class MediaSessionCallback : MediaSessionCompat.Callback() {
    override fun onPlay() {
      if (mediaPlayer?.isPlaying == false) {
        mediaPlayer?.start()
        setMediaPlaybackState(PlaybackStateCompat.STATE_PLAYING, mediaPlayer?.currentPosition?.toLong())
        startUpdatingProgress(mediaPlayer!!)
      }
    }
    ...
    override fun onStop() {
      ...
      stopUpdatingProgress()
      Log.d("QuranAudioService", "onStop")
    }

    override fun onSkipToNext() {
      val currentMediaIndex = mediaItems.indexOf(loadedMediaItem)
      if (currentMediaIndex >= 0) {
        mediaPlayer?.stop()
        val nextMediaItem = mediaItems[currentMediaIndex + 1]
        prepareMediaItem(nextMediaItem)
        loadedMediaItem = nextMediaItem
      }
    }
